...
#	print(get_avg_power(), " ", keys.mic_base)
	can_hear = get_avg_power() > keys.mic_base
	if playtest:
		can_hear = Input.is_action_pressed("scream")

# give the position of player to monster
# i wanna do somethingextra but got no time maybe next time.
	if can_hear:
		pass
#		print("yes")
#		screen_cover.hide()
